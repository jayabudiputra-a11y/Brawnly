...
const _VC = ["#facc15","#f87171","#60a5fa","#4ade80","#c084fc","#f472b6","#fb923c","#2dd4bf"];
...
function _uHx(s: string): string {
  let h = 0;
  for (let i = 0; i < s.length; i++) { h = ((h << 5) - h + s.charCodeAt(i)) | 0; }
  return (h >>> 0).toString(36).slice(0, 8);
}

function _pFI(raw: string|null|undefined): string|null {
  if (!raw) return null;
  const lines = String(raw).split(/[\r\n]+/).map(l => l.trim()).filter(Boolean);
  for (const line of lines) {
    const v = _vU(line.startsWith("http") ? line : null);
    if (v) return v;
  }
  return _vU(lines[0] || null);
}

interface ArticleCardProps { article: any; priority?: boolean; }
...
export default function ArticleCard({ article: _a, priority: _p = false }: ArticleCardProps) {
  ...
  const _hQ = (() => {
    const candidates = [
      _a.featured_image_url,
      _a.featured_image,
      _a.image,
    ];
    for (const c of candidates) {
      if (!c) continue;
      const parsed = _pFI(String(c));
      if (parsed) return _vU(_fC(parsed));
    }
    return null;
  })();
  ...
  const _cK    = _hQ ? `${_a.slug}_${_uHx(_hQ)}` : null;
  const _cp    = _hQ ? _dIS(_hQ) : null;
  ...
  const _jL = {
    ...
    "image":_bIO(_hQ,`${_t} — cover image`,`Cover image for article: ${_t}`,true),
    ...
    "mainEntityOfPage":{"@type":"WebPage","@id":`${_SU}/article/${_a.slug}`,"url":`${_SU}/article/${_a.slug}`},
  ...
  const _jLLI = {
    "@context":"https://schema.org","@type":"ListItem",
    "item":{
      "@type":"BlogPosting","url":`${_SU}/article/${_a.slug}`,"headline":_t,"name":_t,
      "image":_bIO(_hQ,`${_t} — cover image`,`Cover image for article: ${_t}`,true),
      "author":{"@type":"Person","name":_authorName},
      "datePublished":_a.published_at||_a.created_at||undefined,
      "interactionStatistic":{"@type":"InteractionCounter","interactionType":"https://schema.org/ReadAction","userInteractionCount":_a.views??0},
    },
  };

  const _dI = _blobUrl || _hQ;
  ...
  return (
    <article
      ...
      onMouseEnter={() => _sRC(_VC[Math.floor(Math.random() * _VC.length)])}
      style={{ "--hover-color": _rC } as React.CSSProperties}
    >
      <script type="application/ld+json">{JSON.stringify(_jL)}</script>
      <script type="application/ld+json">{JSON.stringify(_jLLI)}</script>

      <meta itemProp="headline"    content={_t} />
      <meta itemProp="name"        content={_t} />
      <meta itemProp="description" content={_a.excerpt||_a.description||`Read ${_t} on Brawnly.`} />
      <meta itemProp="url"         content={`${_SU}/article/${_a.slug}`} />
      {_a.published_at && <meta itemProp="datePublished" content={_a.published_at} />}
      {(_a.updated_at||_a.published_at) && <meta itemProp="dateModified" content={_a.updated_at||_a.published_at} />}
      {_a.category && <meta itemProp="articleSection" content={_a.category} />}
      {_hQ && <meta itemProp="image" content={_hQ} />}
      <meta itemProp="interactionStatistic" content={`ReadAction:${_a.views??0}`} />

      <div aria-hidden="true" style={{position:"absolute",width:1,height:1,overflow:"hidden",clip:"rect(0,0,0,0)",whiteSpace:"nowrap"}}>
        <a href={`${_SU}/article/${_a.slug}`} itemProp="url" tabIndex={-1} rel="noopener noreferrer">{_t}</a>
        <span itemScope itemType="https://schema.org/Person" itemProp="author">
          <span itemProp="name">{_authorName}</span>
          <a href={_SU} itemProp="url" tabIndex={-1} rel="noopener noreferrer">Brawnly Editorial</a>
        </span>
        ...
        {_hQ && _cp && (
          <figure itemScope itemType="https://schema.org/ImageObject" itemProp="image">
            <img src={_hQ} alt={`${_t} — cover image`} itemProp="url" tabIndex={-1} width={400} height={260} />
        ...
        {_a.category && <span itemProp="articleSection">{_a.category}</span>}
        {_a.tags && <span itemProp="keywords">{Array.isArray(_a.tags)?_a.tags.join(", "):_a.tags}</span>}
        {(_a.excerpt||_a.description) && <p itemProp="description">{_a.excerpt||_a.description}</p>}
        <span itemScope itemType="https://schema.org/Blog" itemProp="isPartOf">
          <a href={_SU} itemProp="url" tabIndex={-1} rel="noopener noreferrer">Brawnly Blog</a>
          <span itemProp="name">{_SN}</span>
        </span>
      </div>
      ...
      >
        <div className="relative flex-shrink-0 w-[110px] h-[110px] md:w-[200px] md:h-[130px] overflow-hidden bg-neutral-100 dark:bg-neutral-900 rounded-xl border-2 border-transparent group-hover:border-[var(--hover-color)] transition-all duration-500 shadow-sm">
          <div className="absolute top-2 left-2 z-20 flex gap-1">
            {_oF && (
              <div className="bg-red-600 p-1 rounded-md shadow-lg animate-pulse" title="Offline">
                <_Wo size={10} className="text-white" aria-hidden="true" />
              </div>
            )}
            {_blobUrl && (
              <div className="bg-yellow-500 p-1 rounded-md shadow-lg" title="Optimized">
                <_Zp size={10} className="text-black" aria-hidden="true" />
              </div>
            )}
          </div>

          {!_iL && <div className="absolute inset-0 bg-neutral-200 dark:bg-neutral-800 animate-pulse" aria-hidden="true" />}
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-3 text-[9px] font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-400">
              <span className="text-black dark:text-white group-hover:text-[var(--hover-color)] transition-colors" itemProp="author">
                By {_authorName}
              </span>
              <span className="flex items-center gap-1" aria-label={`${_a.views??0} views`}>
                <_E className="w-3 h-3 text-[#00a354] group-hover:text-[var(--hover-color)] transition-colors" aria-hidden="true" />
                {_a.views ?? 0}
              </span>
            </div>
          </div>
      ...
      <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-32 h-32 bg-[var(--hover-color)] opacity-5 rounded-full blur-3xl group-hover:opacity-10 transition-all duration-500" aria-hidden="true" />
    </article>
  );
}