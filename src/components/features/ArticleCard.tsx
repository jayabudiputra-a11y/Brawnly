...
export default function ArticleCard({ article }: ArticleCardProps) {
  ...
  const title = article[`title_${lang}`] ?? article.title_en ?? article.title ?? "";
  
  const firstImagePath = article.featured_image_path_clean
    ? article.featured_image_path_clean.split("\r\n")[0]?.trim()
    : null;

  const highQualityUrl = firstImagePath ? generateFullImageUrl(firstImagePath) : null;
  const isLowQualityMode = isEnabled && saveData.quality === "low";
  const displayUrl = isLowQualityMode && highQualityUrl ? getLowQualityUrl(highQualityUrl) : highQualityUrl;
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-3 text-[9px] font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-400">
              {/* Author name: Dinamis */}
              <span className="text-black dark:text-white">By {article.author || "Putra Jaya"}</span>
              <span className="flex items-center gap-1">
                <Eye className="w-3 h-3 text-[#00a354]" /> {article.views ?? 0}
              </span>
            </div>
          </div>
        </div>
