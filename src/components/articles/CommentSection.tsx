...
const CommentSection: React.FC<{ articleId: string }> = ({ articleId }) => {
  ...
  // Ref untuk mengarahkan scroll tepat ke form
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setContent("");
      setReplyTo(null);
      toast.success(replyTo ? "Balasan terkirim!" : "Komentar terkirim!");
      await loadComments();
    } catch (err: any) {
  ...
  return (
    <div className="mt-10 border-t dark:border-neutral-800 pt-10">
      ...
      {isAuthenticated ? (
        <form 
          ref={formRef} // 2. Pasang Ref di sini
          onSubmit={handleSubmit} 
          className="mb-10 scroll-mt-32" // Beri jarak scroll agar tidak tertutup Navbar
        >
          {replyTo && (
          ...
          <textarea 
            ...
            placeholder={replyTo ? `Tulis balasan untuk ${replyTo.name}...` : "Tulis pendapatmu..."}
            value={content}
        ...
        <div className="p-6 bg-gray-50 dark:bg-neutral-900 rounded-xl text-center mb-10 border dark:border-neutral-800">
          ...
          <a href="/signin" className="text-emerald-600 font-bold uppercase underline">Masuk Disini</a>
        </div>
      )}
      ...
      <div className="space-y-8 min-h-[400px]">
        {currentRootComments.length > 0 ? (
            ...
            <div key={c.id} className="group">
              <div className="flex gap-4 p-4 bg-white dark:bg-neutral-900 border dark:border-neutral-800 rounded-xl shadow-sm">
                ...
                <div className="flex-1">
                  <p className="font-black text-xs text-emerald-600 dark:text-emerald-400">{c.user_name}</p>
                  <p className="text-gray-700 dark:text-gray-200 mt-1 leading-relaxed">{c.content}</p>
                  <div className="flex gap-4 mt-3">
                    <button 
                      onClick={() => {
                        setReplyTo({ id: c.id, name: c.user_name });
                        // 3. Scroll cerdas ke form input
              ...
              <div className="ml-8 md:ml-12 mt-4 space-y-4 border-l-2 border-gray-100 dark:border-neutral-800 pl-4">
                {getReplies(c.id).map((reply) => (
                  <div key={reply.id} className="flex gap-3 p-3 bg-gray-50/50 dark:bg-neutral-800/30 rounded-lg border dark:border-neutral-800">
                    <img 
                      src={reply.user_avatar_url || `https://ui-avatars.com/api/?name=${reply.user_name}&background=10b981&color=fff`} 
                      className="w-8 h-8 rounded-full shadow-sm"
                      alt={reply.user_name}
                    />
                    <div>
                      <p className="font-bold text-[11px] text-emerald-600 dark:text-emerald-400">{reply.user_name}</p>
          ...
          <p className="text-center text-gray-400 py-20">Belum ada diskusi di sini.</p>
        )}
      </div>
